...
.PHONY: external-setup external-check
...
external-setup:
	$(UV) sync --project reproductions/langgraph-8834 --locked

external-check:
	$(UV) lock --project reproductions/langgraph-8834 --check --offline
	$(RUN) python scripts/check_external.py
